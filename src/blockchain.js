...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
          try {
            ...
            let currentTiem = parseInt(new Date().getTime().toString().slice(0, -3));

            if( (currentTiem - messageTime) > 300){
              reject("There is a timing conflict, elapsed time longer than 5 min ...")
            }
            let verified = await bitcoinMessage.verify(message, address, signature);
            if(!verified){
              reject("There was a problem with the verification")
            }
     ...
     * 3. Execute the validateChain() function every time a block is added    
     * 4. Create an endpoint that will trigger the execution of validateChain()
     */
    validateChain() {
        let self = this;
        let errorLog = [];
        return new Promise(async (resolve, reject) => {
          try {
            await Promise.all(self.chain.map( async block => {
              // validate genesis block, !genesis ? validate()
              if(block.height === 0){
